...
import { addCouponApi } from "../../common/services";
...
function AddModal({ onClose, fetchCoupons }) {
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    try {
      ...
      if (values.maxDiscount && values.type === "PERCENT") {
        data.maxDiscount = Number(values.maxDiscount);
      }
      if (values.minOrderValue) {
        data.minOrderValue = Number(values.minOrderValue);
      }
      if (values.expiresAt) {
      ...
      if (values.perUserLimit) {
        data.perUserLimit = Number(values.perUserLimit);
      }
              ...
              <div className="grid grid-cols-1 gap-5">
                ...
                <div>
                  ...
                  <Field
                    ...
                    placeholder="Enter coupon code (e.g., NEWYEAR50)"
                    onChange={(e) => {
