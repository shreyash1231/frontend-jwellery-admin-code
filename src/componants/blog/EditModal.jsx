...
import { updateBlogApi } from "../../common/services";
...
const EditModal = ({
  ...
  const [open, setOpen] = useState(showUpdateBlog);
  const [loading, setLoading] = useState(false);
  const [imagePreview, setImagePreview] = useState(selectedBlog?.image || null);
  const [selectedImage, setSelectedImage] = useState(null);

  const handleCloseModal = () => {
    setOpen(false);
    setShowUpdateBlog(false);
    setImagePreview(selectedBlog?.image || null);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm, setSubmitting }) => {
      try {
        ...
        const response = await updateBlogApi(selectedBlog.id, formData);
        if (response?.success) {
      ...
      <div className="bg-white rounded-2xl shadow-lg w-[900px] h-[80vh] overflow-y-auto">
        ...
        <form onSubmit={formik.handleSubmit} className="p-8">
          ...
          <div className="w-full mb-6">
            ...
            <div className="mt-4">
              <img
                src={`${image_url}/${selectedBlog?.imageUrl || imagePreview}`}
                alt="Preview"
