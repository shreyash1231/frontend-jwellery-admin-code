...
import { getAllQueriesApi } from "../../common/services";
...
const Queries = () => {
  ...
  const fetchQueries = async () => {
    try {
      const res = await getAllQueriesApi();
      if (res?.success) {
        // support either res.data.queries or res.data
  ...
  const filtered = useMemo(() => {
    ...
    return queries.filter((it) => {
      const name = `${it?.name || it?.user_name || ""}`.toLowerCase();
      const email = `${it?.email || ""}`.toLowerCase();
      const subject = `${it?.subject || it?.title || ""}`.toLowerCase();
      const message = `${it?.message || it?.description || ""}`.toLowerCase();
      return (
  ...
  return (
    <div className="p-4 mt-16">
      <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
        <h2 className="text-2xl font-bold text-black">Queries</h2>
        <input
          type="text"
          value={search}
          onChange={(e) => {
